...
#define LI_PKTFIFO_SIZE 1000
static uint8 ayPktFIFO[LI_PKTFIFO_SIZE];
...
*/
static void LI1_Enqueue(uint8 yByte)
{
  ayPktFIFO[nPktFIFOWritePtr++] = yByte;
  if (nPktFIFOWritePtr >= LI_PKTFIFO_SIZE)
  {
    nPktFIFOWritePtr = 0;
  }
}

static uint8 LI1_Dequeue(void)
{
  uint8 yByte = ayPktFIFO[nPktFIFOReadPtr++];
  if (nPktFIFOReadPtr >= LI_PKTFIFO_SIZE)
  {
    nPktFIFOReadPtr = 0;
  }
...
static uint8 LI1_Peek(void)
{
  uint8 yByte = ayPktFIFO[nPktFIFOPeekPtr++];
  if (nPktFIFOPeekPtr >= LI_PKTFIFO_SIZE)
  {
    nPktFIFOPeekPtr = 0;
  }
...
static int LI1_BytesInQueue(void)
{
  int nCount = nPktFIFOWritePtr - nPktFIFOReadPtr;
  
  if (nCount < 0)
  {
    nCount+=LI_PKTFIFO_SIZE;
  }
  return(nCount);
}
...
{
  if (OP_GetTXEnable()) //Before it was checking for yTXEnable
  {
...
char  szVal[4];
  ...
  while (!(cdhio->STATUS1 & STS1_LI1_RX_EMPTY))
  {
    ...
    sprintf(szVal,"%02X ",yByte);   //Unsigned hexadecimal integer
  ...
  switch (pBuf[3])
  {
    ...
    case GET_TRANSCEIVER_CONFIG:
      if (wPayloadLen == sizeof(RADIO_CONFIGURATION_t))
      {
        ...
        /*
        strncpy(szCall1,rcLastConfig.dest,6);
        szCall1[6] = 0;
        strncpy(szCall2,rcLastConfig.src,6);
        szCall2[6] = 0;
        
        print_uart_msg("LI1: %s/%s  RxFreq=%d  TxFreq=%d  Power=%d\n",
                        szCall1,szCall2,Swap32(rcLastConfig.rx_freq),Swap32(rcLastConfig.tx_freq),
                        rcLastConfig.tx_power_amp_level);
        */
      }
      break;
...
void LI1_GetLastRadioConfig(RADIO_CONFIGURATION_t *prcConfig)
{
  memcpy(prcConfig,&rcLastConfig,sizeof(RADIO_CONFIGURATION_t));
}
