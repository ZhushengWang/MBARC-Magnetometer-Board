...
uint32 unInitialWait;
  ...
  {   
    switch (yCmdID)
    {
      ...
      case RESET_CDH:
        ...
        while(1)  //Wait until watchdog expires
        {
          ;
        }
        break;
      ...
      case FIPEX_LOAD_SCRIPT:
        //print_uart_msg("FIPEX:  Loading Script\n");
        FIP_StoreScriptBytes(pBuf[3],pBuf[4],pBuf[5],&pBuf[6]);
        break;
        
      case FIPEX_VERIFY_SCRIPT:
        FIP_VerifyScript(pBuf[3]);
        break;
        
      case FIPEX_SET_ACT_SCRIPT:
        OP_SetActiveFIPScript(pBuf[3]);
        print_uart_msg("FIPEX:  Active script set to %d (%lu)\n",pBuf[3],get_time());
        break;
        
      case FIPEX_PRINT_SCRIPT:
        FIP_PrintScript(pBuf[3]);
        break;
        
      case FIPEX_LIST_SCRIPT:
        FIP_ListScripts();
        break;
        
      case FIPEX_ERASE_SCRIPT:
        FIP_EraseScript(pBuf[3]);
        break;
        
      case FIPEX_SCRIPT_PROC:
        FIP_SetScriptProcEnable(pBuf[3]);
        break;
        
      case FIPEX_ERASE_DATA:
        unTime = makeui32_be(pBuf,4);
        FIPData_EraseData(pBuf[3],unTime);
        break;
        
      case FIPEX_RETRIEVE_DATA:
        TLM_SendFIPPacket(pBuf[3]);
        break;
        
      case FIPEX_INIT_RETRIEVAL:
        FIPData_InitRetrieval();
        break;
  ...
  {
    print_uart_msg("Bad Length: %d (%lu)\n",wLen,get_time());
