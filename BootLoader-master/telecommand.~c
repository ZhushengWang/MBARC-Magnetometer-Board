...
int nNumBytes;
  ...
  {   
    switch (yCmdID)
    {
      ...
      case TEST_PORT_WRITE_BYTES:
        yAddr = pBuf[3];
        //ayData byte[] = pBuf + 4; //trying to get pointer to pBuf?
        //print_uart_msg("Write:  Addr=%02X  Data=%02X\n",yAddr,yData);
        //*((uint8 *)cdhio + yAddr) = yData;
        //PrintBytes(pBuf, nLen);
        for (i = 0; i < wLen - 1; i++)
        {
          *((uint8 *)cdhio + yAddr) = pBuf[4 + i];
        }
        break;
      ...
      case TEST_PORT_READ:
        ...
        //only handles MBARC magnetometer data
      case TEST_PORT_READ_BYTES:
        yAddr = pBuf[3];
        nNumBytes = 0;
        while(!(cdhio->STATUS1 & STS1_MBARC_MAG_RX_EMPTY))
        {
          cdhio->MBARC_MAG_RX = 0;
          ayWrite_Bytes_Data[nNumBytes++] = *((uint8 *)cdhio + yAddr);
        }
        //print_uart_msg("Read:  Addr %02X = %02X\n",yAddr, yData);
        PrintBytes(ayWrite_Bytes_Data, nNumBytes);
        break;
